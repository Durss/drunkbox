...
//==============================================
  ...
  const int buttonLedPin = 8;//Button's led
  const int sensorPin = A0;//Pin where the gas sensor is plugged
  const int potPin = A1;//Pin where the potentiometer is
...
unsigned int calibrateCountDown = 0;
...
void loop() {
  ...
  int potValue = floor(rawPotValue/potSteps);
  ...
  if(digitalRead(buttonPin) == 0) {
    if(!wasButtonPressed) delay(100);//debounce
  ...
  //If potentiometer is used.
  //Need to move two steps away to enable update. LED screen
  //has impact on the pot's value (dunno why...) so this "2 steps"
  //is a way to avoid displaying a pot change if it's just a
  //fluctuation due to the LED matrix.
  int diff = abs(rawPotValue - prevPotValue);
  if((diff >= potSteps * 2 && calibrateCountDown == 0) ||
     (diff >= potSteps && calibrateCountDown > 0) ) {
    matrix.clear();
    float level = map(potValue, 0, 1000/potSteps, 0, 8*8);
    int lines = floor(level/8);
    for(int i = 0; i < lines; i++) {
      matrix.drawPixel(2, 7-i, LED_ON);
      matrix.drawPixel(3, 7-i, LED_ON);
    }
    lines = round(level)%8;
    for(int i = 0; i < lines; i++) {
      matrix.drawPixel(4, 7-i, LED_ON);
      matrix.drawPixel(5, 7-i, LED_ON);
    }
    matrix.writeDisplay();
    prevPotValue = rawPotValue;
    calibrateCountDown = 20;
  }
